...
| **自定义时间窗** | 除预设窗口外，可指定起止日期精确框定区间（如 2008–2010），日期选择器受实际数据覆盖范围约束 |
| **趋势线** | 可选的移动平均叠加线，窗口长度随数据频率自动缩放 |
| **关系分析** | Relationship 标签页：主序列与对比序列的散点图（按时间着色）、最小二乘拟合线、R²、斜率、t 统计量与相关系数 |
...
添加对比序列后出现 **Relationship** 标签页：

- **散点图**：每个点是两个序列都有观测的一个日期，按日期渐变着色——颜色较深者更近。这样可以直接看出关系是否发生了**结构性变化**（例如菲利普斯曲线的整体位移），而单一相关系数无法揭示这一点。
- **最小二乘拟合**：报告 R²、斜率、t 统计量与样本数。拟合线只画在实际观测的 x 区间内，不向外延伸，避免暗示区间外的预测。
- **量纲化解读**：斜率以「每单位 X 对应多少单位 Y」的形式给出，两个坐标轴各自使用**自身单位**——因为最有意义的散点图恰恰是那些单位不可比的组合（失业率 vs 通胀）。
- **诚实性约束**：斜率若小于其自身标准误的两倍，界面会明确提示「至多是提示性的」；并始终注明拟合描述的是同期共动，不构成因果证据。
